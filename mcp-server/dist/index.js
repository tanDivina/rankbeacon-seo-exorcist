...
import { wsManager, messageRouter } from './communication/index.js';
...
import { predictorTool } from './tools/predictor.js';
import { algorithmDetectionTool } from './tools/algorithm.js';
...
const logger = winston.createLogger({
    ...
    format: winston.format.combine(winston.format.timestamp(), winston.format.errors({ stack: true }), winston.format.json(), winston.format.printf(({ timestamp, level, message, ...meta }) => {
        ...
        return `${emoji} [${timestamp}] ${level.toUpperCase()}: ${message} ${Object.keys(meta).length ? JSON.stringify(meta) : ''}`;
    })),
    transports: [
    ...
    try {
        switch (name) {
            ...
            case 'analyze_competitors':
                return await analyzeCompetitorsTool(args);
            ...
            case 'predict_rankings':
                return await predictorTool.handler(args);
            case 'detect_algorithm_impact':
                return await algorithmDetectionTool.handler(args);
